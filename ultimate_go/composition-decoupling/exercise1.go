...
func (s sysadmin) administrate(system string) {
  println(s.name, "administrate", system)
}
...
func main() {
  ...
  var admins adminlist
	...
	admins.Enqueue(sysadmin{name: "Bob"})

	// Enqueue two new programmers onto devs.
	devs.Enqueue(programmer{name: "Rob"})
	devs.Enqueue(programmer{name: "Edgar"})
